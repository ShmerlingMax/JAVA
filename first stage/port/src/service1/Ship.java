...
public class Ship
{
    ...
    public void print()
    {
        ...
        System.out.printf("%-20s", StringUtils.right("0"+day_, 2) + ":" + StringUtils.right("0" + hours_, 2)
                + ":" + StringUtils.right("0" + minutes_, 2));
        switch (typeOfCargo_)
        {
            case 0 -> System.out.printf("%-17s", "сыпучий");
            case 1 -> System.out.printf("%-17s", "жидкий");
            case 2 -> System.out.printf("%-17s", "контейнера");
        }
        System.out.printf("%-22s", cargoQuantity_);
        System.out.println(StringUtils.right("0"+(plannedStayingPeriod_/60)/24,2)+":" + StringUtils.right("0"
                +((plannedStayingPeriod_/60)-((plannedStayingPeriod_/60)/24)*24),2)+":"+StringUtils.right("0"
                +plannedStayingPeriod_%60,2));
    }
    public void printStatistics()
    {
        System.out.printf("%-25s", name_);
        System.out.printf("%-35s", StringUtils.right("0"+day_, 2) + ":" + StringUtils.right("0" + hours_, 2)
                + ":" + StringUtils.right("0" + minutes_, 2));
        System.out.printf("%-35s", StringUtils.right("0"+(waitingTime_/60)/24,2)+":" + StringUtils.right("0"
                +((waitingTime_/60)-((waitingTime_/60)/24)*24), 2)+":"+StringUtils.right("0"+waitingTime_%60, 2));
        System.out.printf("%-35s", StringUtils.right("0"+(unloadingStartTime_/60)/24,2)+":" + StringUtils.right("0"
                +((unloadingStartTime_/60)-((unloadingStartTime_/60)/24)*24),2)+":"+StringUtils.right("0"
                +unloadingStartTime_%60,2));
        System.out.println(StringUtils.right("0"+(durationOfUnloading_/60)/24,2)+":" + StringUtils.right("0"
                +((durationOfUnloading_/60)-((durationOfUnloading_/60)/24)*24),2)+":"+StringUtils.right("0"
                +durationOfUnloading_%60,2));
    }
